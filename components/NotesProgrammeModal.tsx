...
import { createProgramme } from "@/lib/data/programmes";
...
export default function NotesProgrammeModal({ onCreated, onClose }: {
  ...
  const [programmeName, setProgrammeName] = useState("New programme");
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await supabase.from("programmes").update({ name: programmeName }).eq("id", programme.id);

      // Insert programme sessions
      ...
      const { error: insertErr } = await supabase.from("programme_sessions").insert(rows);
      if (insertErr) throw insertErr;

      // Remove the default empty session created by createProgramme
      await supabase.from("programme_sessions").delete().eq("programme_id", programme.id).eq("sort_order", 0).eq("name", "Session 1").lte("id", rows[0] ? "zzzz" : "0");

      onCreated();
      router.push(`/programmes/${programme.id}`);
    } catch (e) {
      setError(e instanceof Error ? e.message : "Could not save programme");
      setPhase("review");
    }
  };
    ...
    <>
      ...
      <div style={s.overlay} onClick={onClose}>
        <div style={s.modal} onClick={(e) => e.stopPropagation()}>
          <div style={s.header}>
            <span style={s.title}>📝 New programme from notes</span>
          ...
          {phase === "parsing" && (
            <div style={s.centre}>
              <div style={{ fontSize: 28, color: "var(--accent)", animation: "np-spin 0.9s linear infinite", display: "inline-block" }}>⟳</div>
          ...
          {phase === "review" && (
            <>
              <div style={s.metaRow}>
                <div style={{ flex: 1 }}>
                  <div style={s.fieldLabel}>Programme name</div>
                  <input value={programmeName} onChange={(e) => setProgrammeName(e.target.value)} style={s.input} />
                </div>
              </div>
              <div style={s.sessionCount}>{sessions.length} session{sessions.length !== 1 ? "s" : ""} detected</div>
          ...
          {phase === "saving" && (
            <div style={s.centre}>
              <div style={{ fontSize: 28, color: "var(--accent)", animation: "np-spin 0.9s linear infinite", display: "inline-block" }}>⟳</div>
              <div style={s.spinLabel}>Saving programme…</div>
            </div>
          )}
        </div>
      </div>
...
const s: Record<string, React.CSSProperties> = {
  ...
  sessionCount: { fontSize: 12, color: "var(--accent)", fontWeight: 600 },
