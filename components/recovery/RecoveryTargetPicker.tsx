...
export default function RecoveryTargetPicker({
  selectedIds,
  onChange,
}: {
  selectedIds: string[];
  ...
  useEffect(() => {
    ...
    listGroupMembers(selectedGroupId)
      .then((members) => onChange(members.map((m) => m.athlete_id)))
        ...
        <div style={s.checkList}>
          {athletes.map((a) => (
            <label key={a.id} style={s.checkRow}>
              <input type="checkbox" checked={selectedIds.includes(a.id)} onChange={() => toggleAthlete(a.id)} style={{ accentColor: "#2DD4BF" }} />
              {a.name}
            </label>
          ))}
        </div>
...
const s: Record<string, React.CSSProperties> = {
  ...
  tabBtnActive: { background: "#123832", color: "#2DD4BF" },
  ...
  checkList: { display: "flex", flexDirection: "column", gap: 4, maxHeight: 200, overflowY: "auto" as const, border: "1px solid var(--line)", borderRadius: 8, padding: 8 },
