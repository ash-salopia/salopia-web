...
export const HELP_ARTICLES: HelpArticle[] = [
  ...
  },
  {
    ...
    body: [
      ...
      { type: "p", text: "Tap **Metrics** on the card and tick what this exercise logs — you can pick several at once. **Load (kg)** and **Reps** are one value per set; **Time, Distance (m), Distance (cm), Height, Velocity, Power, RSI, Contact time** are one value per rep. Examples: a sled sprint = Load + Time + Distance (m); a med-ball throw = Load + Reps + Distance (cm); a drop jump = Height + Contact time (RSI fills in automatically). Movement type (Acceleration / Max Velocity / Power / Plyometric / COD / Deceleration) seeds a sensible default set." },
      ...
      { type: "subhead", text: "Power vs. Plyometric" },
      { type: "p", text: "These are separate on purpose. **Plyometric** is fast, reactive stretch-shortening-cycle work — depth jumps, hurdle hops, repeated bounds — where the quick, elastic ground contact is the point. **Power** is everything else ballistic and explosive without that reactive element: a broad jump, a CMJ, a med-ball throw, a loaded jump squat. Getting this right matters for the session summary bar's **Plyo contacts** total, which only counts Plyometric-quality exercises (automatically, as sets × reps)." },
      { type: "p", text: "A Plyometric exercise also gets a **Count contacts** tickbox (on by default) for the rare case it shouldn't add to that total at all, and **Additional contacts per rep** for when one rep is itself several ground contacts (a rapid multi-hop drill). Set either as a library default so you only make the call once per exercise." },
